...
class DocumentAnalyzerAgent:
    ...
    async def analyze(self, claim_data: Dict[str, Any], documents: List[str] = None, claim_id: Optional[str] = None) -> AgentResult:
        """
        Analyze documents for a claim
        
        Args:
            claim_data: Claim data dictionary
            documents: Optional list of document paths (if not provided, will fetch from claim_id)
            claim_id: Optional claim ID to fetch documents from storage
        """
        # If claim_id provided, fetch documents from storage
        if claim_id and not documents:
            document_paths = file_storage.get_absolute_paths(claim_id)
            documents = document_paths
        
        # Fallback to documents from claim_data
        ...
        if documents:
            for i, doc_path in enumerate(documents):
                try:
                    ...
                    doc_summary.append(f"Document {i+1}: {Path(doc_path).name}\nExtracted Text: {ocr_result[:200]}...")
                except Exception as e:
                    doc_summary.append(f"Document {i+1}: {Path(doc_path).name} (OCR failed: {str(e)})")
        else:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            Provide a confidence score (0-1) for document validity."""),
            ...
            RECOMMENDATIONS: [Comma-separated list of missing or additional documents needed]""")
        ])
    ...
    async def _extract_text_with_ocr(self, document_path: str) -> str:
        ...
        Supports: images (jpg, png), PDFs, and URLs
        """
        try:
            # Check if it's a URL
            ...
            if self.vision_model:
                ...
                Include dates, amounts, names, addresses, and any other important information.
                If this is a medical record, invoice, or official document, extract all relevant fields."""
                
                response = self.vision_model.generate_content([prompt, image])
                extracted_text = response.text
                
                return extracted_text if extracted_text else "[No text detected in document]"
            else:
                # Fallback: basic file info if no API key
                return f"[OCR not available - Gemini API key required. File: {Path(document_path).name}]"
